...
2026-09-22 / ADR-015 / PharmaScount 品牌与全站备案。
Pharma 路由使用独立品牌资源与元数据，保留 DeerFlow 的名称、图标和业务内容。
备案栏位于 Next.js 根布局，全屏区域为其预留高度。海报作为独立 PNG 交付，
README 保持不变。演示初始化兼容旧品牌工作区名，原账户、数据库及服务标识
不变，避免新增重复工作区。

2026-09-22 / ADR-016 / 用户报告本机资源不足并撤销测试与部署授权。
停止进一步测试、构建、部署，只整理并提交推送代码；停止本项目四个服务。
保留数据，禁用可管理的用户级服务自启动。系统级服务禁用需管理员鉴权，已在
部署交接记录中明确，不通过修改应用或反复杀进程绕过该权限边界。
